...
type ColumnKey = 'bib' | 'name' | 'wave_start' | 'race_kit' | 'row' | 'row_no' | 'shirt' | 'shirt_type' | 'gender' | 'nationality' | 'age_category' | 'block' | 'pre_order' | 'first_half_marathon' | 'note' | 'top_50_no' | 'top50' | 'colour_sign' | 'vip' | 'qr' | 'first_half' | 'bib_pass_link';
...
const RunnerTable: React.FC<RunnerTableProps> = ({ refreshDataTrigger }) => {
  ...
  const areFormsEqual = useCallback((obj1: Partial<Runner>, obj2: Runner | null, rawIdCard?: string): boolean => {
    ...
    const keysToCompare: Array<keyof Runner> = [
      ...
      "top_50_no", "top50", "colour_sign", "vip", "qr", "first_half", "id_card_hash"
    ];
            ...
            <table className="min-w-full divide-y divide-gray-700 bg-gray-900 rounded-md">
              <thead className="bg-gray-700">
                <tr>
                  ...
                  {visibleColumns.has('vip') && <th scope="col" className="px-4 py-2 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">VIP</th>}
              ...
              <tbody className="divide-y divide-gray-700">
                {runners.map((runner) => (
                  <tr key={runner.id} className="hover:bg-gray-700">
                    ...
                    {visibleColumns.has('vip') && <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-200">{runner.vip}</td>}
          ...
          <>
            ...
            {isEditingRunner && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                />
                <div>
                  <label htmlFor="edit-vip" className="block text-sm font-medium text-gray-300 mb-1">VIP</label>
                  <select
                    id="edit-vip"
                    name="vip"
                    value={(editForm.vip || '').toUpperCase()}
                    onChange={handleEditFormChange}
