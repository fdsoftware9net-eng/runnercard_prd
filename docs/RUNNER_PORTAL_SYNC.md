...
2. **ค่า `shirt_size` ที่เขารับ** — ข้อมูลเรามี `2XS` ด้วย ซึ่งเอกสารไม่ได้ระบุรายการค่าที่รับ
   ถ้าเขาปฏิเสธ จะเห็นเป็น `rejected` พร้อมเหตุผลรายคน แล้วค่อยตกลงตาราง mapping กัน
3. **secret ตัวจริง** และ **sandbox key** ยังไม่ได้รับ
4. **วันตัดโอน** (cutover) ที่เขาจะปิด endpoint — ขอทราบล่วงหน้าเพื่อแจ้งผู้จัด
